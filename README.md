...
- For fixtures whose expected channel covers the body (`both` or `body` — the two asterpay routes today), the body is watched as well, and the per-route log line carries `body_match` alongside `challenge_match`. The body is compared as the object `payment_required.extract_payment_required()` pulls out of it — the same extraction the validator and the fixture tests run — not as raw bytes, so key order and whitespace are not drift while an added or changed field is. A `both` fixture makes two live claims and the header matching does not vouch for the body: asterpay drifted on the body alone on 2026-09-01 (`watch-reports/2026-09-01-asterpay-body.md`).
- An empty route list is itself a finding: `git ls-tree` exits 0 with empty stdout on a path that no longer exists, so a moved `tests/fixtures/` would otherwise log `0/0 fully matched` and exit 0. The watch logs `FINDING: route list empty (fixtures path missing?)` and exits non-zero instead — blindness must not report as success.
